...
| 13 | [x] | `optimization/report.py`：`OptimizationReport` / `OptimizationRound` 结构化报告（JSON + text summary）；`build_report` 从 `TopologyVersion` 生成；CLI 增 `optimize` 子命令（`--topology --scenario --start-version --end-version --format text|json --out`）；新测试 `tests/unit/test_opt_report.py`（7 项） |